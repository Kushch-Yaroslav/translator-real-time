...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate recorded English test phrases against the local EN->RU pipeline."
    )
    parser.add_argument("--dataset-dir", default="recordings/test_phrases")
    parser.add_argument("--output-json", default="recordings/test_phrases/evaluation.json")
...
def transcribe_with_realtime_stt(
    ...
    try:
        ...
        for start in range(0, len(audio), chunk_samples):
            ...
            if chunk.size == 0:
                continue
            service.send_audio_chunk(chunk.reshape(-1, 1), samplerate)
    ...
    return service.get_last_final_text().strip()
...
def main() -> int:
    ...
    translation_service = TranslationService(
        TranslationConfig(direction=TranslationDirection.EN_TO_RU, enabled=True)
    )

    print("Loading TTS voice...")
    tts_service = TTSService(
        TTSConfig(voice_name="ru_RU-dmitri-medium", data_dir=args.tts_data_dir, use_cuda=None)
    )
