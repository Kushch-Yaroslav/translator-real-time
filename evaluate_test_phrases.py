...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Directory with metadata.json and wav files.",
    )
    parser.add_argument(
        "--output-json",
        default="recordings/test_phrases/evaluation.json",
        help="Where to save the evaluation report.",
    )
    parser.add_argument(
        "--stt-url",
        default="http://localhost:9000",
        help="Base URL for local realtime STT service.",
    )
    parser.add_argument(
        "--stt-ws-url",
        default="ws://localhost:9000/v1/realtime?intent=transcription",
        help="WebSocket URL for local realtime STT service.",
    )
    parser.add_argument(
        "--tts-data-dir",
        default="/media/yaroslav/DATA/ai_models/piper",
        help="Piper voice model directory.",
    )
    parser.add_argument(
        "--silence-threshold",
        type=float,
        default=0.006,
        help="RMS threshold used for trimming leading and trailing silence.",
    )
    parser.add_argument(
        "--padding-ms",
        type=float,
        default=120.0,
        help="How much audio to keep around detected speech boundaries.",
    )
